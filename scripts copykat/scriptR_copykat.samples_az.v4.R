#####################################################################
#####################################################################
# Run CopyKAT CNV prediction (on single samples, RPCA, CCA, Harmony integrated rds object)
#####################################################################
#####################################################################
...
library(proxy)         # needed for cosine dist
library(grid)
library(svpluscnv)     # for CNV frequency viz

#####################################################################
#####################################################################

# Load and register BiocParallel with multicore support
library(BiocParallel)

# Prevent BiocParallel from being overridden on package load
options(BiocParallel.registerOnLoad = FALSE)
options(BiocParallel.snow.port = function() 0L)  # Disable port allocation

# Register safe parallel backend
...
#####################################################################
#####################################################################
# Load session : load("all_methods.RData") 

obj = readRDS("merged_AG_afCB_afDF_res0.3_annotation_050625.rds")
...
illness = "project_AZDYS_entire_sample"

output_dir <- paste0(illness, ".copykat.output", sep="")
...
#####################################################################
#####################################################################
# Step 2: Run CopyKAT without known normal cells
#####################################################################
#####################################################################
...
copykat_result <- copykat(
  ...
  n.cores         = 10,
  sam.name = file.path(output_dir, "")                   # the sample name, and the output folder
  # smooth.output = TRUE,
  # sam.name        = ""
)
...
##########################################################################
########################################################################## the new code to display UMAP

# ===============================================
# CHECK/CREATE UMAP REDUCTION
# ===============================================

cat("\n=== CHECKING UMAP REDUCTION ===\n")

if ("umap" %in% names(obj_copykat@reductions)) {
  cat("✓ UMAP reduction found\n")
  umap_dims <- ncol(obj_copykat@reductions$umap@cell.embeddings)
  cat("UMAP dimensions:", umap_dims, "\n")
} else {
  cat("! UMAP reduction not found. Creating UMAP...\n")
  
  # Check if PCA exists
  if ("pca" %in% names(obj_copykat@reductions)) {
    cat("✓ PCA found. Running UMAP on PCA...\n")
    obj_copykat <- RunUMAP(obj_copykat, reduction = "pca", dims = 1:30, verbose = FALSE)
    cat("✓ UMAP created successfully\n")
  } else {
    cat("! PCA not found. Running PCA first...\n")
    obj_copykat <- RunPCA(obj_copykat, verbose = FALSE)
    obj_copykat <- RunUMAP(obj_copykat, reduction = "pca", dims = 1:30, verbose = FALSE)
    cat("✓ PCA and UMAP created successfully\n")
  }
}


# ===============================================
# CREATE COPYKAT UMAP PLOTS
# ===============================================

cat("\n=== CREATING COPYKAT UMAP PLOTS ===\n")
...
# Main CopyKAT prediction plot
p1 <- DimPlot(obj_copykat, 
              reduction = "umap", 
...
# Save the main plot
main_plot_file <- file.path(output_dir, "copykat_UMAP_predicted_aneuploidy.png")
...
cat(" Saved main plot:", main_plot_file, "\n")
...
# The first 3 columns in the CNV matrix are the genomic coordinates. 
# Rows are 220KB bins in genomic orders. 
# It also output CNVs indexed by gene names and other information such as 
# chromosome names, start and end positions, G staining band etc

# chrompos → for plotting within individual chromosomes.
# abspos → for genome-wide plots (e.g. heatmaps, Manhattan plots) 
# where x-axis is continuous across chromosomes.
...
# cna_raw_df <- read.table("RPCA_copykat_CNA_raw_results_gene_by_cell.txt", 
...
methods <- c("euclidean", "manhattan", "minkowski", "cosine")
...
################################################################################################
######################################################## from seg file 
######################################################## compute CNV frequency
######################################################## adding the analysis on the seg file!
######################################################## package cnvplussv
################################################################################################
################################################################################################
################################################################################################
...
seg_file <- file.path(output_dir, "_copykat_CNA_results.seg")
...
} # end of the script with cnvpluscv package

#####################################################################
##################################################################### VIOLIN PLOT
####################################################### log2R boxplot : aneuploid and diploid cells
##################################################################### experimental
#####################################################################
...
# seg.mean <- -0.5
# estimated_CN <- 2 * 2^seg.mean  # ~1.41
...
# gistic2(
#  seg = "./RPCA_copykat_CNA_results.seg",  # Path to your SEG file
...
# That’s a critical observation, and it happens more often than you’d expect in CopyKAT 
# and other scRNA-based CNV inference tools. Let’s break it down.
